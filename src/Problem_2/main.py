...
if __name__ == '__main__':
    ...
    for cnt in range (3):
     ...
     rf_sim_matrix = relevance_feedback(vec_docs, vec_queries, sim_matrix1)
     sim_matrix1=rf_sim_matrix
     if args.verbose:
    ...
    for cnt in range (3):
     print ("iter:",cnt)
     rf_sim_matrix = relevance_feedback_exp(vec_docs, vec_queries, sim_matrix2, tfidf_model)
     sim_matrix2=rf_sim_matrix
     if args.verbose:
        for i in range(rf_sim_matrix.shape[1]):
            ranked_documents = np.argsort(-rf_sim_matrix[:, i])
            print ('Query:', i+1, 'Top relevant 10 documents:', ranked_documents[:10] + 1)
...
# notes
#  we have to update the similarity matrix , using the inferences from ground truth
